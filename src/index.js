...
var handlers = {
  ...
  DescriptionSearchIntent: function() {
    ...
    getDescriptionList(this.event.request.intent, this.emit);
  },
  SetPreferencesIntent: function() {
    var speechOutput =
      "Ok, What layout preference do you prefer?  Please say 'laptop' or 'desktop'";
...
var setLayoutPreferenceHandlers = Alexa.CreateStateHandler(
  states.LAYOUT_PREFERENCE_MODE,
  {
    CaptureLayoutPreferenceIntent: function() {
      var layoutPreference = this.event.request.intent.slots.LayoutPreference
        .value;

      if (layoutPreference === "laptop" || layoutPreference === "desktop") {
        this.attributes["layoutPreference"] = layoutPreference;
        var speechOutput =
          "Ok, your layout preference is " + layoutPreference + ".";
...
function getDescriptionList(intent, speechCallback) {
  var speechOutput = getDescriptionListItems(intent.slots.OperationName.value);
  speechCallback(
    ":tell",
    speechOutput,
    getRepromptText("word within a command description")
  );
}
...
function findClosestMatchingDescription(operation) {
  ...
  dataModel[`${version}`].forEach(function(item) {
    ...
    if (distance < shortestDistance) {
      ...
      console.log(
        "Shorter Levenshtein Distance Match Found in Clean Description: " +
          item["description"] +
          " - " +
          distance
      );
